          ...
          <YMaps query={{ apikey: "cd6e05b5-779b-4303-b21c-f9534e4a4a39" }}>
            ...
            >
              ...
              {filteredPoints.map((point) => {
                ...
                // --- Генерируем SVG с текстом и хвостиком на лету! ---
                // Создаем красивую рамку, выравниваем текст и добавляем хвостик вниз.
                // Ширина подстраивается под текст (примерно 10px на букву + отступы).
                // --- Генерируем SVG с текстом и хвостиком на лету! ---
                const textWidth = Math.max(120, point.name.length * 9 + 32); 
                // Вычисляем координаты для хвостика ровно по центру
                const mid = textWidth / 2;
                
                // Рисуем единый контур (path) со скругленными углами (радиус 8) и хвостиком внизу
                const svgLabel = `
                ...
                return (
                  <React.Fragment key={point.id}>
                    ...
                    {/* 2. Если маркер выбран - рисуем НАД НИМ нашу SVG-картинку с названием */}
                    {isSelected && (
                      <Placemark 
                        geometry={[parseFloat(lat), parseFloat(lng)]}
                        options={{ 
                          ...
                          // Смещаем ровно по центру (половина ширины) и вверх над основным маркером
                          iconImageOffset: [-(textWidth / 2), -90], 
                          interactive: false, // Отключаем клики по самой надписи
                          zIndex: 1001
                        }}
                      />
                    )}
